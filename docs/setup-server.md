...
リクエストのタイムアウトや履歴保持に関する設定を変更できます。

```env
# リクエストタイムアウト（秒、デフォルト 120）
# フックが timeout を指定しない場合のデフォルト値
# REQUEST_TIMEOUT=120

# リクエスト自動削除（秒、デフォルト 300）
# 応答済み・期限切れのリクエストがこの時間で削除される
# REQUEST_CLEANUP=300
...
`REQUEST_TIMEOUT` はフックが `timeout` を指定しない場合のデフォルト値です。フック側で `PROMPT_RELAY_TIMEOUT` を設定している場合はそちらが優先されます。
...
# セカンダリ用に別のルームキーを使う場合（省略時はプライマリと同じキーを使用）
